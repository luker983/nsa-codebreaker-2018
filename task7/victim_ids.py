...
def main():
    try:
        ...
        pos = str(0).rjust(64, '0')
        storage_key = to_hex(w3.sha3(hexstr=pos))
        user = w3.toHex(w3.eth.getStorageAt(w3.toChecksumAddress(ESCROW_ADDRESS), storage_key))
        print("User Address:", user)
        print(w3.fromWei(w3.eth.getBalance(w3.toChecksumAddress(ESCROW_ADDRESS)), 'ether'))
        ...
        for v in victims:
            ...
            print("Dec Key:", w3.toHex(w3.eth.getStorageAt(w3.toChecksumAddress(ESCROW_ADDRESS), storage_key)))
